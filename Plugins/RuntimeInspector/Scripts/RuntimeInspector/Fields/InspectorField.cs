...
#pragma warning disable 0649
        ...
        [SerializeField] protected Text variableNameText;

        [SerializeField] protected Image variableNameMask;

        [SerializeField] private MaskableGraphic visibleArea;
#pragma warning restore 0649
        ...
        public string Name
        {
            get
            {
                if (variableNameText) return variableNameText.text;
                return string.Empty;
            }
            set
            {
        ...
        public string NameRaw
        {
            ...
            set
            {
                if (variableNameText) variableNameText.text = value;
            }
        }
        ...
        public void BindTo(InspectorField parent, MemberInfo variable, string variableName = null)
        {
...
#endif
                    BindTo(field.FieldType, variableName, () => field.GetValue(parent.Value),
                        (value) => field.SetValue(parent.Value, value), variable);
                else
            ...
            else if (variable is PropertyInfo)
            {
				...
				if( !parent.BoundVariableType.GetTypeInfo().IsValueType )
#endif
                    BindTo(property.PropertyType, variableName, () => property.GetValue(parent.Value, null),
                        (value) => property.SetValue(parent.Value, value, null), variable);
                else
        ...
        public void Unbind()
        {
            ...
            OnUnbound();
            RuntimeInspectorManager.Instance.PoolDrawer(this);
        }
        ...
        public InspectorField CreateDrawerForVariable(MemberInfo variable, Transform drawArea,
            string variableName = null)
        {
            Type variableType = variable is FieldInfo
                ? ((FieldInfo)variable).FieldType
                : ((PropertyInfo)variable).PropertyType;
            InspectorField variableDrawer =
                Inspector.CreateDrawerForType(variableType, drawArea, Depth + 1, true, variable);
    ...
    {
#pragma warning disable 0649
        [SerializeField] protected RectTransform drawArea;

        [SerializeField] private PointerEventListener expandToggle;
        private RectTransform expandToggleTransform;

        [SerializeField] private LayoutGroup layoutGroup;

        [SerializeField] private Image expandArrow; // Expand Arrow's sprite should look right at 0 rotation
#pragma warning restore 0649
        ...
        public bool IsExpanded
        {
            ...
            set
            {
                ...
                if (expandArrow != null)
                    expandArrow.rectTransform.localEulerAngles =
                        m_isExpanded ? new Vector3(0f, 0f, -90f) : Vector3.zero;
            ...
            set
            {
                if (m_headerVisibility != value)
                {
                    ...
                    if (m_headerVisibility == RuntimeInspector.HeaderVisibility.Collapsible)
                    {
                        ...
                        variableNameText.rectTransform.sizeDelta =
                            new Vector2(-(Skin.ExpandArrowSpacing + Skin.LineHeight * 0.5f), 0f);
                    }
        ...
        {
            if (Inspector.ShowRemoveComponentButton && typeof(Component).IsAssignableFrom(BoundVariableType) &&
                !typeof(Transform).IsAssignableFrom(BoundVariableType))
            ...
            if (methods != null)
            {
                ...
                for (int i = 0; i < methods.Length; i++)
                {
                    ...
                    if ((isInitialized && method.VisibleWhenInitialized) ||
                        (!isInitialized && method.VisibleWhenUninitialized))
        ...
        {
            InspectorField variableDrawer =
                Inspector.CreateDrawerForType(component.GetType(), drawArea, Depth + 1, false);
            if (variableDrawer != null)
            {
                if (variableName == null)
        ...
        {
            InspectorField variableDrawer = CreateDrawerForVariable(variable, drawArea, variableName);
            if (variableDrawer != null)
            {
                elements.Add(variableDrawer);
            }
        ...
        public InspectorField CreateDrawer(Type variableType, string variableName, Getter getter, Setter setter,
            bool drawObjectsAsFields = true)
        {
            InspectorField variableDrawer =
                Inspector.CreateDrawerForType(variableType, drawArea, Depth + 1, drawObjectsAsFields);
            if (variableDrawer != null)
            {
        ...
        {
            ExposedMethodField methodDrawer =
                (ExposedMethodField)Inspector.CreateDrawerForType(typeof(ExposedMethod), drawArea, Depth + 1, false);
