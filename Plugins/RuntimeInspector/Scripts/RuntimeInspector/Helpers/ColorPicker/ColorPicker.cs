...
#pragma warning disable 0649
        ...
        [SerializeField] private ColorWheelControl colorWheel;

        [SerializeField] private ColorPickerAlphaSlider alphaSlider;

        [SerializeField] private Text rgbaText;

        [SerializeField] private BoundInputField rInput;

        [SerializeField] private BoundInputField gInput;

        [SerializeField] private BoundInputField bInput;

        [SerializeField] private BoundInputField aInput;

        [SerializeField] private LayoutElement rgbaLayoutElement;

        [SerializeField] private LayoutElement buttonsLayoutElement;
...
#pragma warning restore 0649
        ...
        public void Show(ColorWheelControl.OnColorChangedDelegate onColorChanged,
            ColorWheelControl.OnColorChangedDelegate onColorConfirmed, Color initialColor, Vector2 position,
            Vector2 offset,
            Canvas referenceCanvas = null)
        {
            ...
            RectTransformUtility.ScreenPointToLocalPointInRectangle(this.transform as RectTransform,
                ...
                out var localPoint);
            position = localPoint;
            var panelRectTransform = panel.rectTransform;
            var canvasRect = (RectTransform)transform;

            var sizeDelta = panelRectTransform.sizeDelta;
            sizeDelta.y = -sizeDelta.y;
            position += sizeDelta * panelRectTransform.pivot;
            position += offset;
            if (position.x + panelRectTransform.sizeDelta.x*panelRectTransform.pivot.x  > canvasRect.rect.xMax)
                position.x -= panelRectTransform.sizeDelta.x+ offset.x*2;
            if (-position.y + panelRectTransform.sizeDelta.y*panelRectTransform.pivot.y > canvasRect.rect.yMax)
                position.y += panelRectTransform.sizeDelta.y - offset.y*2;
