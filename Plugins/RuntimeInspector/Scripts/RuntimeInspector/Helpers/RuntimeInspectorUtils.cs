...
namespace RuntimeInspectorNamespace
{
    public static class RuntimeInspectorUtils
    {
        ...
        private static readonly Dictionary<Type, ExposedMethod[]> typeToExposedMethods =
            new Dictionary<Type, ExposedMethod[]>(89);
        ...
        private static readonly List<ExposedExtensionMethodHolder> exposedExtensionMethods =
            new List<ExposedExtensionMethodHolder>();

        public static Type ExposedExtensionMethodsHolder
        {
            set { GetExposedExtensionMethods(value); }
        }

        private static Dictionary<Type, Type> customEditors;

        private static readonly List<RuntimeInspectorCustomEditorAttribute> customEditorAttributes =
            new List<RuntimeInspectorCustomEditorAttribute>(4);
        ...
        private static readonly Stack<DraggedReferenceItem> draggedReferenceItemsPool =
            new Stack<DraggedReferenceItem>();
        ...
        internal static readonly NumberFormatInfo numberFormat =
            NumberFormatInfo.GetInstance(CultureInfo.InvariantCulture);
        ...
        public static string ToTitleCase(this string str)
        {
            ...
            for (; index < str.Length; index++)
            {
                ...
                if (char.IsUpper(ch))
                {
                    if ((lastCharType < 2 || (str.Length > index + 1 && char.IsLower(str[index + 1]))) &&
                        stringBuilder.Length > 0)
        ...
        internal static string GetNameWithType(this object obj, Type defaultType = null)
        {
            ...
            return (obj is Object)
                ? string.Concat(((Object)obj).name, " (", obj.GetType().Name, ")")
                : obj.GetType().Name;
        }
        ...
        public static void ShowTooltip(string tooltip, PointerEventData pointer, UISkin skin = null,
            Canvas referenceCanvas = null)
        {
            ...
            if (!tooltipPopup)
            {
                tooltipPopup = (Tooltip)Object.Instantiate(Resources.Load<Tooltip>("RuntimeInspector/Tooltip"),
                    popupCanvas.transform, false);
        ...
        public static DraggedReferenceItem CreateDraggedReferenceItem(Object reference,
            PointerEventData draggingPointer, UISkin skin = null, Canvas referenceCanvas = null)
        {
            return CreateDraggedReferenceItem(new Object[1] { reference }, draggingPointer, skin, referenceCanvas);
        }

        public static DraggedReferenceItem CreateDraggedReferenceItem(Object[] references,
            PointerEventData draggingPointer, UISkin skin = null, Canvas referenceCanvas = null)
        {
            ...
            {
                referenceItem = (DraggedReferenceItem)Object.Instantiate(
                    Resources.Load<DraggedReferenceItem>("RuntimeInspector/DraggedReferenceItem"),
                    popupCanvas.transform, false);
                hasCanvasChanged = true;
            }
        ...
        public static object GetAssignableObjectFromDraggedReferenceItem(PointerEventData draggingPointer,
            Type assignableType)
        {
            return GetAssignableObjectsFromDraggedReferenceItemInternal(draggingPointer, assignableType, true);
        }

        public static object[] GetAssignableObjectsFromDraggedReferenceItem(PointerEventData draggingPointer,
            Type assignableType)
        {
            return (object[])GetAssignableObjectsFromDraggedReferenceItemInternal(draggingPointer, assignableType,
                false);
        }

        private static object GetAssignableObjectsFromDraggedReferenceItemInternal(PointerEventData draggingPointer,
            Type assignableType, bool returnFirstObject)
        {
#if ENABLE_INPUT_SYSTEM && !ENABLE_LEGACY_INPUT_MANAGER
                ...
                case RenderMode.WorldSpace:
                    ...
                    else
                    {
                        ...
                        Vector3 centerOffset =
                            new Vector3((0.5f - referenceCanvasTransform.pivot.x) * referenceCanvasRect.width,
                                (0.5f - referenceCanvasTransform.pivot.y) * referenceCanvasRect.height, 0f);
        ...
        public static MemberInfo[] GetAllVariables(this Type type)
        {
            ...
            for (int i = typesToSearchForVariablesList.Count - 1; i >= 0; i--)
            {
                ...
                PropertyInfo[] properties = currType.GetProperties(BindingFlags.Public | BindingFlags.NonPublic |
                ...
                for (int j = 0; j < properties.Length; j++)
                {
                    ...
                    if (property.HasAttribute<ObsoleteAttribute>() || property.HasAttribute<NonSerializedAttribute>() ||
                        property.HasAttribute<HideInInspector>())
                        continue;
                ...
                FieldInfo[] fields = currType.GetFields(BindingFlags.Public | BindingFlags.NonPublic |
                                                        BindingFlags.Instance | BindingFlags.DeclaredOnly);
                for (int j = 0; j < fields.Length; j++)
                {
                    ...
                    if (field.HasAttribute<ObsoleteAttribute>() || field.HasAttribute<NonSerializedAttribute>() ||
        ...
        public static ExposedMethod[] GetExposedMethods(this Type type)
        {
            ...
            if (!typeToExposedMethods.TryGetValue(type, out result))
            {
                ...
                for (int i = 0; i < methods.Length; i++)
                {
                    ...
                    RuntimeInspectorButtonAttribute attribute =
                        methods[i].GetAttribute<RuntimeInspectorButtonAttribute>();
                ...
                for (int i = 0; i < exposedExtensionMethods.Count; i++)
                {
                    ...
                    if (exposedExtensionMethod.extendedType.IsAssignableFrom(type))
                        exposedMethodsList.Add(new ExposedMethod(exposedExtensionMethod.method,
                            exposedExtensionMethod.properties, true));
                }
        ...
        private static void AddCustomEditorInternal(Type customEditorType, bool showWarnings)
        {
            ...
            {
                if (showWarnings)
                    Debug.LogWarning("Type doesn't implement IRuntimeInspectorCustomEditor interface: " +
                                     customEditorType);

                return;
            }

            RuntimeInspectorCustomEditorAttribute[] attributes =
                (RuntimeInspectorCustomEditorAttribute[])Attribute.GetCustomAttributes(customEditorType,
                    typeof(RuntimeInspectorCustomEditorAttribute), false);
        ...
        public static IRuntimeInspectorCustomEditor GetCustomEditor(Type type)
        {
            if (customEditors == null)
            {
                ...
                foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
                {
                    ...
                    {
                        if (caseInsensitiveComparer.IsPrefix(assemblyName, ignoredAssemblies[i],
                    ...
                    {
                        Debug.LogError("Couldn't search assembly for RuntimeInspectorCustomEditor attributes: " +
            ...
            if (!customEditors.TryGetValue(type, out customEditorType))
            {
                ...
                {
                    if (customEditorAttributes[i].EditorForChildClasses &&
                        customEditorAttributes[i].InspectedType.IsAssignableFrom(type))
                    {
